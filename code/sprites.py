...
class MonsterSprite(pygame.sprite.Sprite):
    def __init__(self, pos, frames, groups, monster, index, pos_index, entity, apply_atack, create_monster):
        # data
        self.index = index
    ...
    def animate(self, dt):
        ...
        if self.state == 'attack' and self.frame_index >= len(self.frames['attack']):
            self.apply_atack(self.target_sprite, self.current_attack, self.monster.get_base_damage(self.current_attack))
            self.state = 'idle'
...
class MonsterLevelSprite(pygame.sprite.Sprite):
    def __init__(self, entity, pos, monster_sprite, groups, font):
        ...
        self.rect = self.image.get_frect(topleft=pos) if entity == 'player' else (
            self.image.get_frect(topright=pos))
